...
export const meals = (meals, goals) => {
  $(".meal-food").remove();
...
const dateTotals = (meals, goals) => {
  mealGoals(meals, goals);
  grandTotals(meals, goals)
}

const mealGoals = (meals, goals) => {
  goals.forEach(goal => {
    let calories = meals[goal.name] == undefined ? 
      goal.calories :
      meals[goal.name].calorie_goal
    $(`#${goal.name}-goal`).text(goal.calories);
  })
}

const grandTotals = (meals, goals) => {
  let caloriesConsumed = totalCaloriesConsumed(meals);
  let goalCalories = totalGoalCalories(meals, goals)
  clearGrandTotals(); 
...
const totalGoalCalories = (meals, goals) => {
  let mealArray = ["Breakfast", "Lunch", "Dinner", "Snack"]
  let totalGoalCalories = 0;
  goals.forEach(goal => {
    if(meals[goal.name] == undefined) {
      totalGoalCalories += goal.calories
    } else {
      totalGoalCalories += meals[goal.name].calorie_goal
    }
  });
