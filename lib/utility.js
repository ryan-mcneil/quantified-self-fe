...
export const getMeal = (meals, date, mealName) => {
  let result = meals.find(obj => {
    return obj.date.substring(0,10) == date && obj.name == mealName
  })
  return result
}
...
export const addFoodToMeal = (meal, food) => {
  if(meal.id && food.id) {
    api.postMealFoods(meal.id, food.id)
      .then(response => render.foodsFeedback(response.message))
      .catch(error => render.foodsFeedback(error.message));
  } else {
...
const updateMealContainerId = (meal) => {
  let mealContainer = $(`h3:contains("${meal.name}")`).parent();

  mealContainer.attr("id", `${meal.name}-${meal.id}`)
}